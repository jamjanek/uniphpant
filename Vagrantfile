...
# Configure Apache
echo "<VirtualHost *:80>
	DocumentRoot \"/var/www/api/default/public\"
	AllowEncodedSlashes On

	ServerName "api.uniphpant.local.vm";
	ServerAlias "www.api.uniphpant.local.vm";

	<Directory \"/var/www/api/default/public\">
	...
	ErrorLog /var/www/var/logs/apache2-api-error.log
   	CustomLog /var/www/var/logs/apache2-api-access.log combined

</VirtualHost>" > /etc/apache2/sites-available/000-api.conf
...
cp /var/www/api/default/.env.dist /var/www/api/default/.env
...
echo "** Visit http://localhost:8889 ot http://api.uniphpant.local.vm in your browser for to view the application **"
...
</VirtualHost>" > /etc/apache2/sites-available/000-spa.conf
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    ...
    config.vm.define "api", autostart: false do |api|
        api.vm.network "forwarded_port", guest: 80, host: 8889
        api.vm.network "forwarded_port", guest: 22, host: 8229
        api.vm.network :private_network, :auto_network => true
        api.vm.synced_folder './', '/var/www/', id:"app-root",owner:"vagrant",group:"www-data",mount_options:["dmode=777,fmode=666"]
        api.vm.synced_folder './api', '/var/www/api', id:"api",owner:"vagrant",group:"www-data",mount_options:["dmode=777,fmode=666"]
        api.vm.provision 'shell', inline: @script_api
        api.vm.hostname = 'api.uniphpant.local.vm'

        api.vm.provider "virtualbox" do |vb|
            vb.customize ["modifyvm", :id, "--memory", "1024"]
            vb.customize ["modifyvm", :id, "--name", "uniphpant-api-default"]
        end
    end
